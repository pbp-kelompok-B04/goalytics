...
    // --- Main Container for Event Delegation (Click Handler) ---
...
        if (deleteButton) {
            // CRITICAL FIX: Stop event bubbling immediately to prevent double submission
            e.preventDefault();
            e.stopPropagation();
...
                    console.log("Delete response (Success path):", data);
                    
                    let removeTarget = data.removeTarget;

                    if (!removeTarget) {
                        // Predict the removal target based on the URL (for prediction delete)
                        const urlParts = url.split('/');
                        // Assumes URL structure is /.../prediction/{id}/delete/
                        const predictionId = urlParts[urlParts.length - 3]; 
                        removeTarget = `#prediction-${predictionId}`; 
                    }

                    if (removeTarget) {
                        const target = document.querySelector(removeTarget);
                        if (target) {
                            console.log("Removing target:", removeTarget);
                            target.remove();
                        } else {
                            console.warn("Remove target not found:", removeTarget);
                            // Fallback to reload if row removal fails
...
                } else {
                    // This block catches 4xx/5xx errors, including the 404 from the second request.
                    const text = await resp.text();
                    console.error("Delete failed (Error path):", resp.status, text);

                    // FIX: Only show an error toast if the status is NOT an expected 404/403
                    if (resp.status !== 404 && resp.status !== 403) {
                        showToast("Delete failed", "error"); 
                    }
                }
            } catch (err) {
                // FIX: Ensure 'data' is not referenced here. Only use literal messages.
                console.error("Error deleting item (Network or Unexpected Parse Error):", err);
                showToast("Error deleting item. Check connection.", "error");
            }
            return;
        }
    }); 

    // --- Handle AJAX Form Submission (Submit Handler) ---
